...
function computeModulePackageName(moduleName, packageName, old) {
  return old ? `./${moduleName}` : `@restapp/${decamelize(moduleName)}-${packageName}`;
}
...
function addSymlink(packageName, modulePackageName) {
  fs.symlinkSync(
    ...
    `${BASE_PATH}/node_modules/@restapp/${decamelize(packageName)}-${modulePackageName}`
  );
}
...
function removeSymlink(packageName, modulePackageName) {
  fs.unlinkSync(`${BASE_PATH}/node_modules/@restapp/${decamelize(packageName)}-${modulePackageName}`);
}
